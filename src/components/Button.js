...
const renderContent = (color, disabled, indicator, text) => {
  if (indicator) {
    return <ActivityIndicator color={colors.white} style={[styles.centering]} />;
  }

  return (
    <Text
      style={[styles.defaultText, color ? { color } : null, disabled ? styles.disabledText : null]}
    >
      {text}
    </Text>
  );
...
const Button = ({
  backgroundColor, block, color, disabled, indicator, onPress, text,
}) => {
